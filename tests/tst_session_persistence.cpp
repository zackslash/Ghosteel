...
private slots:
    ...
    void testKeepAwakeReleasedOnSessionRemoval()
    {
        ...
        // The empty-list fallback in removeSession() then creates a fresh shell,
        // so the list is not empty — but the fallback session is not keep-awake.
        QSignalSpy countSpy(&mgr, &SessionManager::keepAwakeActiveCountChanged);
        mgr.removeSession(0);
        QCOMPARE(mgr.keepAwakeActive(), false);
        QCOMPARE(mgr.keepAwakeActiveCount(), 0);
        QCOMPARE(countSpy.count(), 1);
        QCOMPARE(mgr.sessionCount(), 1); // empty-list fallback creates a fresh shell
        QCOMPARE(mgr.sessionKeepAwake(0), false);
    }
    ...
    void testModelDataRolesContract()
    {
        ...
        // roleNames must expose all seven roles
        ...
        QVERIFY(roles.value(Qt::UserRole + 7) == "keepAwake");
        ...
        QCOMPARE(idx.data(Qt::UserRole + 7).toBool(), false); // keep-awake off by default
