...
const QRegularExpression &urlRegex()
{
    ...
    static QRegularExpression re(
        QStringLiteral(
            R"url((?:(?:https?://|mailto:|ftp://|file:|ssh:|git://|tel:|magnet:|ipfs://|ipns://|gemini://|gopher://|news:)(?:(?:\[[0-9a-fA-F:]+(?:[0-9a-fA-F:]*)+\](?::[0-9]+)?)|[\w\-.~:/?#@!$&*+,;=%]+(?:[\(\[]\w*[\)\]])?)+(?<![,.])|(?:(?<![\w.])(?:\.\.\/|\.\/|\.[\w][\w\-.]*\/)|(?<!\w)~\/|(?:[\w][\w\-.]*\/)*(?<!\w)\$[A-Za-z_]\w*\/|(?<![\w~\/:.])\/(?!\/)(?=[\w\-.~:\/?#@!$&*+;=%]))(?:(?=[\w\-.~:\/?#@!$&*+;=%]*\.)[\w\-.~:\/?#@!$&*+;=%]+(?:(?<!:) (?!\w+:\/\/)(?!\.{0,2}\/)(?!~\/)[\w\-.~:\/?#@!$&*+;=%]*[\/.])*)*(?<!:)(?: +(?= *$))?|(?<![\w\/])[\w][\w\-.]*\/[\w\-.~:\/?#@!$&*+;=%]+\.\w+(?<!:)(?: +(?= *$))?))url"
        )
    );
    return re;
}
