...
# Sailfish OS Emulator Glyph Atlas Render Test
#
# Pushes scripts/glyph-catalog.sh to the emulator and renders it through
# Ghosteel so you can visually validate the GlyphAtlas upload path
# (src/glyphatlas.cpp rasterizeGlyph). This is the regression gate for the
# stride bug: narrow glyphs must stay clean after wide (CJK) glyphs.
#
# Usage: ./emulator_glyph_test.sh           # push catalog + launch in Ghosteel
#        ./emulator_glyph_test.sh --clean    # remove catalog from emulator
...
CATALOG="$SCRIPT_DIR/glyph-catalog.sh"
REMOTE_CATALOG="/tmp/ghosteel-glyph-catalog.sh"
...
echo "============================================================"
echo "  Section A vs D  ASCII must look IDENTICAL. D follows the wide"
echo "                 CJK in Section C, so D is the stride-bug trigger."
echo "                 Horizontal bands / transparent vertical gaps in D = FAIL."
echo "  Section E      Narrow/wide interleaved on one line. Must be clean"
echo "                 with no banding BETWEEN glyphs (hardest case)."
echo "  Section F      Bold and underline render with correct weight."
echo "  Section G      Emoji may show as tofu/blank (OK). Only banding = FAIL."
echo ""
echo "PASS = Sections A and D match; E is clean between glyphs."
echo "FAIL = any narrow glyph after Section C is banded or fragmented."
echo ""
